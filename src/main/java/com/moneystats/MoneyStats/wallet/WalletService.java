...
public class WalletService {
	...
	public List<WalletDTO> getAll(TokenDTO tokenDTO) throws WalletException, AuthenticationException {
		...
		for (int i = 0; i < walletEntities.size(); i++) {
			walletDTO = new WalletDTO(walletEntities.get(i).getName(), walletEntities.get(i).getCategory(),
					walletEntities.get(i).getUser(), walletEntities.get(i).getStatementList());
	...
	public WalletResponseDTO addWalletEntity(TokenDTO tokenDTO, Integer idCategory, WalletDTO walletDTO)
		...
		WalletEntity walletEntity = new WalletEntity(walletDTO.getName(), walletDTO.getCategoryEntity(),
				walletDTO.getUser(), walletDTO.getStatementEntityList());
		walletDAO.save(walletEntity);
		return new WalletResponseDTO(WalletResponse.Type.WALLET_ADDED.toString());
	}
