...
import com.moneystats.authentication.AuthCredentialService;
...
@SpringBootTest(properties = "spring.main.lazy-initialization=true", classes = AuthCredentialService.class)
public class AuthServiceTest {
	...
	void login_shouldThrowWrongCredential() throws Exception {
		...
		AuthenticationException expected = new AuthenticationException(AuthenticationException.Type.WRONG_CREDENTIAL);
		AuthenticationException actual = Assertions.assertThrows(AuthenticationException.class,
	...
	void token_shouldReturnInvalidInput() throws Exception {
		...
		AuthenticationException expected = new AuthenticationException(AuthenticationException.Type.INVALID_TOKEN_DTO);
		AuthenticationException actual = Assertions.assertThrows(AuthenticationException.class,
	...
	void token_shouldReturnUnauthorized() throws Exception {
		...
		Mockito.when(tokenService.parseToken(token))
				.thenThrow(new AuthenticationException(AuthenticationException.Type.UNAUTHORIZED));

		AuthenticationException expected = new AuthenticationException(AuthenticationException.Type.UNAUTHORIZED);
		AuthenticationException actual = Assertions.assertThrows(AuthenticationException.class,
				() -> service.getUser(token));
	...
	void getUsers_adminShouldThrowInvalidInput() throws Exception {
		...
		Mockito.when(tokenService.parseToken(token))
				.thenThrow(new AuthenticationException(AuthenticationException.Type.INVALID_TOKEN_DTO));
	...
	void getUsers_adminShouldThrowIfTheUserIsNotAnAdmin() throws Exception {
		...
		AuthenticationException expected = new AuthenticationException(AuthenticationException.Type.NOT_ALLOWED);
		AuthenticationException actual = Assertions.assertThrows(AuthenticationException.class,
